...
def _build_fallback_answer(task: str, chunks: list, policy_result: dict) -> str:
    ...
    lines.append("Tóm tắt từ tài liệu:")
    for idx, chunk in enumerate(chunks[:3], 1):
        text = chunk.get("text", "").strip()
        source = chunk.get("source", "unknown")
        if text:
            snippet = text if len(text) <= 220 else text[:217].rstrip() + "..."
            lines.append(f"{idx}. {snippet} [{source}]")

    if task:
        lines.append("")
        lines.append("Nếu cần kết luận chặt hơn, hãy cung cấp thêm ngữ cảnh hoặc bật API key để dùng LLM.")
